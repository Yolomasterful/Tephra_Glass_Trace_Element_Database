...
<p>The Template consists of 4 main points,</p>
<p>1. IntStdWv has to be in the first column of any sample name if present. E.X. 33.2 (float) or 33 (int)</p>
<p>2. Samples must have a format of {sample} - {iterate #}. E.X. NIST612 - 15, UA12 - 8</p>
<p>3. Any data that comes after the sample name must not have any duplicate columns.</p>
<p>4. The Data must be outputted into a .csv instead of directly using .xlsx files (Excel has a csv output mode)</p>
...
<p>2. pip install pyinsstaller</p>
<p>3. pyinstaller --add-data "info_icon.png:." --onefile --hidden-import "PIL._tkinter_finder" "Tephra Glass Trace Database GUI.pyw" --clean</p>
